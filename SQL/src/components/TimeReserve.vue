...
async function CreatUserTbale(x, y, z, t) {
  await supabase.from('ReserveTime').insert({ Name: x, Day: y, Period: z, EMAIL: t })
}
  ...
  data.forEach((row) => {
    if (row.Day == ttest2.value) {
      if (row.Period == ttest3.value) {
        if (row.SeatNumber == 0) {
          alert("You can't reserve this day/period")
        } else {
          ;(CurrentSeat = row.SeatNumber),
            CreatUserTbale(test1.value, ttest2.value, ttest3.value, somethingtest1.email),
            (updateseat = CurrentSeat - 1),
            UPDATEVALUES3(updateseat, row.id),
            router.push('/reserve')
        }
      }
    }
  ...
  >
    <form class="testthisthat" @submit.prevent="TOGETHER">
      <div class="somethingtest">
        ...
        <div>
          <input class="inputfield" required type="string" placeholder="Name" v-model="test1" />
          <br />
